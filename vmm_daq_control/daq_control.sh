...
stop() {
    ...
    echo "[INFO] Waiting for remaining conversions in $RUNDIR..."
	...
	remaining=$((npcap - nroot))
        if [ "$remaining" -eq 0 ]; then
            echo "[OK] No more .pcapng files left."
            echo "[INFO] Waiting 10s for final conversions..."
            sleep 10
            break
        fi
    ...
    echo "[STOP] ALINX: Checking status..."
    alinx-sc --config-file config/config_alinx_noThresholds.json --read-link-status
