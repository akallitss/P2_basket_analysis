...
#
# Each sub-run gets its own log, plot and tracks file under --outdir, plus the
# alignment_<run>_<subrun>.json that align_and_track.py writes itself.  A
# summary table is printed at the end and written to <outdir>/summary.tsv.
#
...
printf 'run\tsub_run\tn_matched\tslope_x\tdx_mm\tspread_x\tslope_y\tdy_mm\tspread_y\tangle_x_mrad\tangle_y_mrad\tseconds\tstatus\n' > "$SUMMARY"
...
for run in "${RUNS[@]}"; do
    ...
    for sub_dir in "$run_dir"/*/; do
        ...
        log="$OUTDIR/${run}_${sub}.log"
        extra=()
        [ "$PLOTS" -eq 1 ] && extra+=(--plot "$OUTDIR/${run}_${sub}.png")
        ...
        [ "$TRACKS" -eq 1 ] && extra+=(--save-tracks "$OUTDIR/${run}_${sub}_tracks.npz")
        ...
        # align_and_track.py writes this next to itself, one file per sub-run
        json="$HERE/alignment_${run}_${sub}.json"
        if [ -f "$json" ]; then
            mv -f "$json" "$OUTDIR/"
            json="$OUTDIR/alignment_${run}_${sub}.json"
            "$PY" - "$run" "$sub" "$json" "$dt" <<'EOF' >> "$SUMMARY"
import json, sys
