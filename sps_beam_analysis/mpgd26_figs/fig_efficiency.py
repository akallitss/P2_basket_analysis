...
for det in DETS:
    ...
    lab = det + (f' = {ch.chamber(det, "drift_mesh_scan_1")}'
                 if det == 'P2_IN' else f' = {ch.chamber(det, "drift_mesh_scan_1")}')
...
if len(rep):
    ax.errorbar(rep.mesh_hv, rep.eff, yerr=[rep.eff - rep.lo, rep.hi - rep.eff],
                ...
                label='P2_IN = CERN-built chamber, from 28 Jul — '
                      'drift = mesh + 300 V')
    st.direct_label(ax, rep.mesh_hv.iloc[-1], rep.eff.iloc[-1],
                    'P2_IN (CERN)', st.C_VIO, dx=4, dy=-20)
...
ax.set_title('P2 efficiency vs mesh HV — SPS muons, Ar/CO$_2$/iso 93/5/2\n'
             ...
             'for the CERN-built chamber that took over P2_IN on 28 Jul',
             fontsize=10.5)
st.finish(fig, f'{OUT}/eff_vs_mesh_urw.png')

# --------------------------------------------------------------- F2: eps(drift)
d = u[u.sub_run.str.startswith('drift_')].copy()
d = d[~((d.station == 'P2_IN'))]  # P2_IN parked at 430/630 in the drift half
...
for ax, det in zip(axes, DETS):
    ...
    r = rs[(rs.run == 'drift_mesh_scan_1') & (rs.det == det)
           & (rs.sub_run.str.startswith('meshscan'))].sort_values('mesh_v')
    if len(r):
        ax.plot(r.mesh_v, r.eff_thr100, color=st.C_VIO, marker='^', lw=1.6,
                ls=':', label='raw ZS stream (thr 100 ADC)')
    ax.set_title(det, color=st.DET_COLOR[det])
    ax.set_xlabel('mesh voltage [V]')
    ax.set_ylim(0, 1.02)
axes[0].set_ylabel('efficiency')
axes[0].legend(loc='upper left', fontsize=9)
fig.suptitle('Three efficiency methods on the same mesh scan (drift_mesh_scan_1)',
             fontweight='bold')
...
# ------------------------------------ F5: P2_IN before/after the chamber swap
fig, ax = plt.subplots(figsize=(7.6, 5.2))
g = m[m.station == 'P2_IN'].sort_values('mesh_hv')
ax.errorbar(g.mesh_hv, g.eff, yerr=[g.eff - g.lo, g.hi - g.eff],
            color=st.C_IN, marker='o', lw=2, capsize=2.5,
            label='det4 at P2_IN (25 Jul, uRWELL-referenced)')
for run, lab, ls, mk in [('p2in_hvrange_2', 'CERN-built chamber '
                          '(28 Jul, tag-probe)', '--', '^'),
                         ('p2_mesh_drift_eff_1', 'CERN-built chamber, full '
                          'scan (28–29 Jul, tag-probe)', ':', 's')]:
    t = tp[(tp.run == run) & (tp.probe == 'P2_IN')
           & (tp.prod_sub == 'scan')].sort_values('hv')
    if len(t):
        ax.plot(t.hv, t.eff_corr, color=st.C_VIO, marker=mk, lw=1.8, ls=ls,
                label=lab)
ax.set_xlabel('mesh voltage [V]')
ax.set_ylabel('efficiency')
ax.set_ylim(0, 1.02)
ax.legend(loc='upper left', fontsize=9)
ax.set_title('The P2_IN station held three chambers: det2 (22–23, 26–27 Jul),\n'
             'det4 (24–25 Jul, leaky drift frame) and the CERN-built one '
             '(from 28 Jul)', fontsize=11)
