...
from FanPadDetector import FanPadDetector, strip_to_ch, STRIP_STRATEGIES
...
def load_large_detector_map(map_csv, detector_key, strategy=None):
    """
    Load P2_BASKET strip mapping (Gerber-derived CSV) and resolve
    (connector_number, mec8_connector) + within-half ordering → (vmm_id, ch).

    Square-pad flavour of the large-detector map (schema-compatible with
    load_detector_map / plot_efficiency_map / plot_hit_heatmap). For the true
    Gerber pad rectangles use FanPadDetector.from_mapping_csv instead — the
    map produced here uses the same validated ordering and centroids.

    Ordering (VMM ch = within-connector-half index):
        strategy='reverse'  → ch = 63 − (strip − base)  ** M3-validated **
        strategy='linear' / 'pairswap' → the alternative within-half orders
        strategy=None       → falls back to the detector's registered strategy,
                              else 'reverse'.
    Position uses the validated Gerber pad centroid (pad_cx/pad_cy); the old
    strip-endpoint (x, y) is ~11.5 mm off the true pad centre and is not used.
    size_mm = arc length between adjacent strips (delta_phi × radius); the first
    strip of each connector (delta_phi = 0) gets the connector median.
    ...
    strategy : {'reverse', 'linear', 'pairswap'} or None
        Within-half ordering; None uses the registered strategy (default
        'reverse').
    ...
    strategy    = strategy or cfg.get("strategy", "reverse")
    if strategy not in STRIP_STRATEGIES:
        raise ValueError(f"strategy {strategy!r} not in {STRIP_STRATEGIES}")
    ...
    df = df.dropna(subset=["vmm_id", "strip"])
    df["vmm_id"] = df["vmm_id"].astype(int)

    # VMM channel = within-half index under the validated ordering.
    df["ch"] = df["strip"].astype(int).map(lambda s: strip_to_ch(s, strategy))
    df = df[df["ch"].between(0, 63)].copy()

    # Validated Gerber pad centroid (pad_cx/pad_cy); fall back to x/y only if
    # the centroid columns are absent from an older CSV.
    if {"pad_cx", "pad_cy"}.issubset(df.columns):
        df = df.rename(columns={"pad_cx": "x_mm", "pad_cy": "y_mm"})
    else:
...
def compare_large_detector_orderings(df_ch_eff, map_csv, mec8_to_vmm,
                                     fpc_connectors, run_no, out_dir=None):
    """
    Compare the candidate channel→pad orderings at the fixed cabled assignment.

    Renders the per-channel efficiency map under each ordering so the two
    families can be checked against the beam profile:

      strip-based (VMM ch = within-half index, geometry from the CSV `strip`
      column, immune to the MEC8 2-pin gap):
        - reverse   ** validated against M3 tracks in the cosmic-bench DREAM
                       readout — the recommended default **
        - linear, pairswap  (the two alternative within-half orders)
      pin-based (ch = f(mec8_pin), cannot reproduce the validated order exactly):
        - normal, flipped, back, flipped_back

    Unlike compare_large_detector_orientations (which sweeps the 4 cable
    assignments × 4 pin orientations), the VMM assignment is held fixed here at
    the user-confirmed cabling (mec8_to_vmm); only the within-half ordering
    varies. The per-panel hit-weighted spread (mm) is shown but does not
    discriminate for a wide muon beam.

    Parameters
    ----------
    df_ch_eff : pd.DataFrame
        Per-channel efficiency for the large-detector VMMs
        (columns vmm_id, ch, true_efficiency).
    map_csv : str
        Path to P2_BASKET_mapping.csv.
    mec8_to_vmm : dict
        {(fpc_connector, mec8_connector): vmm_id} — the cabled assignment.
    fpc_connectors : list of int
    run_no : int
    out_dir : str or None
    """
    orderings = [
        ("reverse (validated)", dict(strategy="reverse")),
        ("linear",              dict(strategy="linear")),
        ("pairswap",            dict(strategy="pairswap")),
        ("pin: normal",         dict(orientation="normal")),
        ("pin: flipped",        dict(orientation="flipped")),
        ("pin: back",           dict(orientation="back")),
        ("pin: flipped_back",   dict(orientation="flipped_back")),
    ]

    ncol = 4
    nrow = int(np.ceil(len(orderings) / ncol))
    fig, axes = plt.subplots(nrow, ncol, figsize=(6.5 * ncol, 6 * nrow))
    axes = np.atleast_1d(axes).ravel()

    for ax, (label, kw) in zip(axes, orderings):
        det = FanPadDetector.from_mapping_csv(
            map_csv, mec8_to_vmm=mec8_to_vmm,
            fpc_connectors=fpc_connectors, pad_size_mm=14.0, **kw)

        eff_map = {}
        for _, row in df_ch_eff.iterrows():
            idx = det.channel_map.get((int(row["vmm_id"]), int(row["ch"])))
            if idx is not None:
                eff_map[idx] = max(0.0, float(row["true_efficiency"]))
        ...
        plt.colorbar(sm, ax=ax, shrink=0.7,
                     format=plt.FuncFormatter(lambda x, _: f"{x*100:.3f}%"))
        ax.set_title(f"{label}   spread={spread:.0f} mm",
                     fontsize=10, fontweight="bold")

    for ax in axes[len(orderings):]:
        ax.axis("off")

    fig.suptitle(
        f"P2 Large Detector — channel-ordering comparison  |  Run {run_no}\n"
        f"strip-based (reverse = M3-validated / linear / pairswap)  vs  "
        f"pin-based orientations   (fixed cabled VMM assignment)\n"
        f"spread does not discriminate for a wide muon beam",
        fontsize=11, fontweight="bold",
    )
    plt.tight_layout()
    _save_fig(fig, out_dir, f"ordering_comparison_run{run_no}")
    return fig
...
def main():
    ...
    for det_key in large_detectors:
        ...
        if df_det.empty:
            print(f"  {det_label}: no hits — skipping map")
            continue
        # Prefer the strip-based ordering validated against M3 tracks
        # ('reverse'); fall back to the pin-based orientation if no strategy
        # is registered for this detector.
        strategy    = vmm_mapping[det_key].get("strategy")
        ...
        if df_adc_hist is not None:
            ...
            plot_adc_neighbor_median(
                ...
                vmm_ids=det_vmms, bin_width=adc_bin_width, n_bins=adc_n_bins,
                out_dir=out_dir)
        print(f"  {det_label}: generating channel-ordering comparison "
              f"(reverse vs pin-based)...")
        compare_large_detector_orderings(
