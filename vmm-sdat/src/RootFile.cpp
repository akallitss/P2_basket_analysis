...
RootFile::RootFile(Configuration &config) : m_config(config) {
  ...
  TTree::SetMaxTreeSize(100000000LL);  // 100MB max before auto-save
 ...
 case 1:
  ...
  m_tree_hits->SetAutoFlush(-50000000);  // flush every 5MB of data
  m_tree_hits->Branch("hits", &m_hit);
  m_tree_hits->SetBasketSize("*", 4096); // 4KB baskets instead of 32KB
  break;
  case 10:
  m_tree_clusters_plane = new TTree("clusters_plane", "clusters plane");
  m_tree_clusters_plane->SetDirectory(m_file);
  m_tree_clusters_plane->SetAutoFlush(-50000000);
  m_tree_clusters_plane->Branch("clusters_plane", &m_cluster_plane);
  m_tree_clusters_plane->SetBasketSize("*", 4096);

  break;
  case 11:
  m_tree_hits = new TTree("hits", "hits");
  m_tree_hits->SetDirectory(m_file);
  m_tree_hits->SetAutoFlush(50000);
  m_tree_clusters_plane = new TTree("clusters_plane", "clusters plane");
  m_tree_clusters_plane->SetDirectory(m_file);
  m_tree_clusters_plane->SetAutoFlush(50000);
  m_tree_hits->Branch("hits", &m_hit);
  m_tree_clusters_plane->Branch("clusters_plane", &m_cluster_plane);
  break;
  case 100:
  ...
  m_tree_clusters_detector->SetAutoFlush(-50000000);
  m_tree_clusters_detector->Branch("clusters_detector", &m_cluster_detector);
  m_tree_clusters_detector->SetBasketSize("*", 4096);
  break;
  case 101:
  m_tree_hits = new TTree("hits", "hits");
  m_tree_hits->SetDirectory(m_file);
  ...
  m_tree_clusters_detector =
      new TTree("clusters_detector", "clusters detector");
  m_tree_clusters_detector->SetDirectory(m_file);
  m_tree_clusters_detector->SetAutoFlush(50000);
  m_tree_hits->Branch("hits", &m_hit);
  m_tree_clusters_detector->Branch("clusters_detector", &m_cluster_detector);
  break;
  case 110:
...
void RootFile::AddHits(Hit &&the_hit) {
  ...
  if (m_hits.size() >= 10000) {  // small batches — 10k hits at a time
    SaveHits();
  }
}
...
void RootFile::SaveHits() {
  if (m_hits.size() > 0) {
    for (int n = 0; n < (int)m_hits.size(); n++) {
      m_hit = m_hits[n];
      m_tree_hits->Fill();
    }
    m_hits.clear();
    m_tree_hits->FlushBaskets();  // flush after every batch
  }
}
